...
import { numberFlashcards } from '@/lib/flashcard-data';
import { PlaceHolderImages } from '@/lib/placeholder-images';
import { useSpeech } from '@/hooks/use-speech';
import { useEffect, useState } from 'react';
import { motion } from 'framer-motion';

export default function NumberFlashcardsPage() {
  ...
  return (
    <div className="flex flex-col h-full">
        <Header title="Number Flashcards" />
                          ...
                          <div className="absolute w-full h-full backface-hidden">
                            <Card className="h-full overflow-hidden shadow-2xl bg-card/80 backdrop-blur-sm border-2 border-primary/20">
                              <CardContent className="flex items-center justify-center h-full p-6">
                                <p className="text-9xl md:text-[10rem] font-bold leading-none text-primary">{item.name}</p>
                              </CardContent>
                            </Card>
                          </div>
