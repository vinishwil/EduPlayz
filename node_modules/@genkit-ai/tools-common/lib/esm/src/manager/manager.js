...
export class RuntimeManager {
    ...
    watchers = [];
    healthCheckInterval;
    constructor(telemetryServerUrl, manageHealth, projectRoot, processManager, disableRealtimeTelemetry) {
    ...
    static async create(options) {
        const manager = new RuntimeManager(options.telemetryServerUrl, options.manageHealth ?? true, options.projectRoot, options.processManager, options.disableRealtimeTelemetry);
        await manager.setupRuntimesWatcher();
        await manager.setupDevUiWatcher();
        if (manager.manageHealth) {
            manager.healthCheckInterval = setInterval(async () => await manager.performHealthChecks(), HEALTH_CHECK_INTERVAL);
        }
        return manager;
    }
    async stop() {
    ...
    async runAction(input, streamingCallback, onTraceId) {
        const runtime = input.runtimeId
            ? this.getRuntimeById(input.runtimeId)
            : this.getMostRecentRuntime();
        if (!runtime) {
        ...
        if (streamingCallback) {
            const response = await axios
                ...
                .catch((err) => this.handleStreamError(err, `Error running action key='${input.key}'.`));
            let genkitVersion;
                ...
                response.data.on('end', () => {
                    try {
                        ...
                        if (responseData.error) {
                            ...
                            err.data = {
                                ...responseData.error,
                                stack: (responseData.error?.details).stack,
                                data: {
    ...
    async cancelAction(input) {
        ...
        try {
            const response = await axios.post(`${runtime.reflectionServerUrl}/api/cancelAction`, { traceId: input.traceId }, {
                headers: {
                    'Content-Type': 'application/json',
                },
            });
    ...
    async streamTrace(input, streamingCallback) {
        const { traceId } = input;
        if (!this.telemetryServerUrl) {
            throw new Error('Telemetry server URL not configured. Cannot stream trace updates.');
        }
        const response = await axios
            ...
            .catch((err) => this.httpErrorHandler(err, `Error streaming trace for traceId='${traceId}'`));
        const stream = response.data;
        let buffer = '';
        return new Promise((resolve, reject) => {
    ...
    async addTrace(input) {
        await axios
            ...
            .catch((err) => this.httpErrorHandler(err, 'Error writing trace to store.'));
    }
    ...
    async handleNewRuntime(filePath) {
        try {
            ...
            if (isValidRuntimeInfo(runtimeInfo)) {
                ...
                const fileName = path.basename(filePath);
                if (await checkServerHealth(runtimeInfo.reflectionServerUrl, runtimeInfo.id)) {
    ...
    httpErrorHandler(error, message) {
        ...
        if (error.response) {
            ...
            newError.stack = (error.response?.data).message;
            if ((error.response?.data).message) {
                newError.data.data = {
                    ...newError.data.data,
                    genkitErrorMessage: message,
                    genkitErrorDetails: {
                        stack: (error.response?.data).message,
    ...
    async handleStreamError(error, message) {
        if (error.response &&
    ...
    async performHealthChecks() {
        const healthCheckPromises = Object.entries(this.filenameToRuntimeMap).map(async ([fileName, runtime]) => {
            if (!(await checkServerHealth(runtime.reflectionServerUrl, runtime.id))) {
                await this.removeRuntime(fileName);
            }
        });
